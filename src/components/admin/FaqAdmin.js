...
function FaqAdmin() {
  ...
  const fetchFaqs = async () => {
    try {
      const response = await axios.get(
        `https://mesindigital.xyz/influence-be/faq.php?category=${selectedCategory}`
      );
      setFaqData((prevData) => ({
        ...prevData,
  ...
  const handleDeleteFaq = async (id) => {
    try {
      await axios.delete("https://mesindigital.xyz/influence-be/faq.php", {
        data: { id },
      });
      fetchFaqs();
    } catch (error) {
    ...
    >
      <Row>
        <Col md={4}>
          <Button
            variant={
              selectedCategory === "influencer" ? "primary" : "outline-primary"
            }
          ...
          <Button
            variant={
              selectedCategory === "brand" ? "primary" : "outline-primary"
            }
        ...
        <Col md={8}>
          ...
          {Array.isArray(faqData[selectedCategory]) &&
          ...
          <Form>
            ...
            <Button variant="primary" className="mt-3" onClick={handleAddFaq}>
              Add FAQ
            </Button>
          </Form>
        </Col>
