...
function EditService({ show, handleClose }) {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const influencerId = localStorage.getItem("influencer_id"); // Ambil influencer_id dari localStorage
        if (!influencerId) {
        ...
        const response = await axios.get(
          "https://mesindigital.xyz/influence-be/SetService.php"
        );
        const allServices = response.data.services;

        // Filter hanya service yang memiliki influencer_id yang sesuai
    ...
    try {
      ...
      const response = await axios.post(
        "https://mesindigital.xyz/influence-be/SetService.php",
        {
          ...
          pricePerPost: service.price_per_post,
      ...
      if (response.data.success) {
        ...
        console.error(
          "There was an error editing the service!",
          response.data.error
        );
      }
    } catch (error) {
  ...
  return (
    <Modal show={show} onHide={handleClose}>
      ...
      <Modal.Body>
          ...
          {services.map((service) => (
            <ListGroup.Item key={service.id}>
              {editServiceId === service.id ? (
                ...
                <>
                  <div>
                    <strong>Nama Service:</strong> {service.service_name}
                  </div>
                  <div>
                    <strong>Deskripsi:</strong> {service.description}
                  </div>
                  <div>
                    <strong>Harga:</strong> {service.price_per_post}
                  </div>
                  <div>
                    <strong>Durasi:</strong> {service.duration}
                  </div>
                  <Button
                    variant="warning"
        ...
        <Form>
          <Form.Group controlId="formNewServiceTemplate"></Form.Group>
